    ...
    <TerminalShell userTier={mockUser.tier} currentSection="today">
      <div className="p-4 md:p-8 space-y-8">
        <div className="space-y-2">
          ...
          <p className="text-tr-gray-light text-xs md:text-sm uppercase tracking-[0.25em]">Know what matters before the session starts.</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <div className="tr-card">
            <div className="text-[10px] font-mono text-tr-gray-light uppercase tracking-[0.25em] mb-2">Market State</div>
            ...
            <div className="mt-2 text-[11px] text-tr-gray-light">Confidence {marketStateConfidence}%</div>
          </div>

          <div className="tr-card">
            <div className="text-[10px] font-mono text-tr-gray-light uppercase tracking-[0.25em] mb-2">Threat Level</div>
            <div className="text-xl font-bold uppercase text-tr-red">{threatLevel}</div>
            <div className="mt-2 text-[11px] text-tr-gray-light">Score {Math.round(threat.score)}</div>
          </div>

          <div className="tr-card">
            <div className="text-[10px] font-mono text-tr-gray-light uppercase tracking-[0.25em] mb-2">Best Opportunity</div>
            {bf.bestOpportunity ? (
              <>
                <div className="text-xl font-bold uppercase text-tr-red">{bf.bestOpportunity.asset}</div>
                <div className="mt-2 text-[11px] text-tr-gray-light">Op {bf.bestOpportunity.opportunityScore} • Conf {bf.bestOpportunity.confidenceScore} • Risk {bf.bestOpportunity.riskScore}</div>
              </>
            ) : (
          ...
          <div className="tr-card">
            <div className="text-[10px] font-mono text-tr-gray-light uppercase tracking-[0.25em] mb-2">Agent Conviction</div>
            ...
            <div className="mt-2 text-[11px] text-tr-gray-light">Conviction {agents.convictionScore}</div>
          </div>
        </div>

        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <div className="tr-card xl:col-span-2">
            <div className="text-[10px] font-mono text-tr-gray-light uppercase tracking-[0.25em] mb-3">Tino's Read</div>
            <div className="text-sm text-tr-white leading-relaxed">{tinoRead.marketNarrative}</div>
            <div className="mt-3 text-[11px] text-tr-gray-light">Trap: {tinoRead.trap}</div>
            <div className="mt-2 text-[11px] text-tr-gray-light">What Matters: {tinoRead.whatMatters.join(' • ')}</div>
            <div className="mt-2 text-[11px] text-tr-gray-light">Stand Down: {tinoRead.standDownCondition}</div>
          </div>

          <div className="tr-card">
            <div className="text-[10px] font-mono text-tr-gray-light uppercase tracking-[0.25em] mb-3">Top Ranked Opportunities</div>
              ...
              {opportunities.sort((a: any, b: any) => b.opportunityScore - a.opportunityScore).slice(0, 6).map((o: any, i: number) => (
                <div key={o.asset} className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-[11px] text-tr-gray-light">Op {o.opportunityScore} • Conf {o.confidenceScore}</div>
                  </div>
                  <div className={`text-[11px] font-bold uppercase ${o.status === 'READY' ? 'text-green-400' : o.status === 'AVOID' ? 'text-tr-red' : 'text-tr-gray-light'}`}>{o.status}</div>
                </div>
              ))}
            </div>
          </div>
